...
- 2026-04-21: Reworked Tile Zoom interaction flow so zoom starts closed, selection is armed by a new `Zoom Tile` button in `Panel Controls`, a visible click-a-tile clue is shown while armed, tile click opens zoom on the chosen tile, button re-press re-arms selection for a new tile, and zoom can be closed with a standard top-right `x` button.
- 2026-04-21: Added a new `Control` group in the `Manipulate` tab with `Toggle Tile` and `Toggle Sub-grid` buttons; canvas click/right-click behavior now follows this explicit mode selection, and selecting sub-grid mode auto-enables sub-grid visibility.
- 2026-04-21: Tile Zoom canvas now accepts direct mouse presses; left/right press applies the currently selected `Control` tool (`Toggle Tile` or `Toggle Sub-grid`) to power/selection toggles for the zoomed tile or sub-grid cell.
- 2026-04-21: Corrected Tile Zoom sub-grid press mapping to use the selected tile's global sub-grid coordinate space (with zoom-canvas border compensation), so sub-grid toggles align with the zoomed tile area rather than origin-biased positions.
- 2026-04-21: Unified wheel behavior so Tile Zoom uses the same wheel-control pipeline as the main view; wheel input is now accepted inside the Tile Zoom panel while remaining suppressed over the control panel UI.
- 2026-04-21: Fixed intermittent `Tab` wheel-mode cycling by switching keydown gating from shared hover-state flags to direct control-panel hover checks, making `Tab` behavior stable when moving pointer between background and Tile Zoom.
- 2026-04-21: Added `Identify Zoomed Tile` action (grouped with other identify controls in `Selection Information`) to select/highlight the currently zoomed tile in the main viewport for quick physical panel location.
- 2026-04-21: Compacted `Selection Information` identify controls by shortening button labels and forcing a three-button side-by-side row (`OFF Panels`, `OFF Sub-grid`, `Zoomed Tile`); `Zoomed Tile` now uses toggle-style active state aligned with whether the zoomed tile is selected.
- 2026-04-21: Added `Zoom Sub-grid` selection flow alongside `Zoom Tile`; arming mode now supports tile or sub-grid target hints, and Tile Zoom panel now renders a second right-side sub-grid zoom canvas with matching in-window interaction controls.
- 2026-04-21: Refined Tile Zoom UX by removing bottom info labels, reducing header height, keeping sub-grid zoom pane hidden until a sub-grid target is selected, and changing both zoom buttons to re-arm selection without forcing the zoom panel closed.
- 2026-04-21: Adjusted Tile Zoom sub-grid expansion sizing so sub-grid-open state auto-resizes panel width to fit tile + sub-grid panes by their own aspect-driven widths, preventing temporary tile squish and avoiding equal two-tile-width forcing.
- 2026-04-21: Replaced temporary Tile Zoom width offset tuning with measured horizontal-overflow correction (`scrollWidth - clientWidth`) so sub-grid-open panel sizing is driven by concrete layout factors instead of magic numbers.
- 2026-04-21: Tile Zoom resize-handle now scales zoom content based on drag movement (width and height intent), preserving aspect while allowing practical manual size adjustment without static empty-right growth behavior.
- 2026-04-21: Landscape Tile Zoom width is now hard-clamped to exact content footprint (`left gutter + tile zoom + center gap + sub-grid zoom + right gutter`) to prevent any residual right-side empty-space growth.
...
- 2026-04-21: Changed portrait/landscape switch threshold to the 1:1 aspect boundary and added resize-time panel re-anchor on layout switch so the drag handle remains aligned to the active cursor grab point.
- 2026-04-21: Refined resize-time switch trigger to follow live drag diagonal crossing (`intended outer height >= intended outer width`) so mode flip happens exactly as the handle crosses 1:1 during active resize.
- 2026-04-21: Corrected post-switch resize tracking to use live cursor-to-corner width (`intended outer width`) each mousemove, preventing accelerated continued resize that felt like dragging from a mid-window point.
- 2026-04-21: Disabled Tile Zoom draw-loop width reconciliation during active manual resize, preventing portrait-mode drag acceleration caused by auto-width lock fighting the live handle drag.
- 2026-04-21: Reverted the manual-resize reconciliation guard after regression reports (landscape narrowing and portrait aspect-lock issues), restoring continuous width-lock behavior during resize.
- 2026-04-21: `Identify Zoomed Tile` now also highlights the active zoomed sub-grid selection (when present), keeping tile and sub-grid context visible together during follow-mode.
...
- 2026-04-23: Added an initial `Paint` workflow in `Manipulate`: enable/disable paint mode, pick sub-grid vs tile-area target, choose foreground/background colors, clear painted cells, and paint directly from Tile Zoom/Sub-grid Zoom using left-click foreground and right-click background.
- 2026-04-23: Fixed base-graphic draw regression where a single tile texture was stretched across the whole frame; base and repeat-source canvases now render correctly tiled per panel geometry again, restoring expected checker-size pixel density (e.g. `32px` checkers).
- 2026-04-23: Restored `Full Screen Image` base rendering to draw once across the full canvas frame (not per-panel tiled), while keeping repeat-source extraction/rendering in fullscreen mode based on the full-frame image.
- 2026-04-23: Refined paint granularity so sub-grid paint mode writes a single screen pixel per click (left/right FG/BG) instead of filling the whole active sub-grid step block (e.g. `16x16`).
- 2026-04-23: Paint layer now composites above the base graphic before repeat sampling, so tile/sub-grid repeat modes also repeat painted content; added press-and-hold paint on Tile Zoom/Sub-grid Zoom via drag strokes (left/right FG/BG).
- 2026-04-23: Hardened paint visibility by always compositing the paint layer on top of the final rendered frame in all modes, while still including paint in repeat-source sampling.
- 2026-04-23: Moved paint controls into a left-side vertical toolbar inside `Tile Zoom` (instead of `Manipulate` panel), matching pixel-editor workflows and keeping painting interactions scoped to the zoom window canvases.
- 2026-04-23: Redesigned Tile Zoom paint toolbar to square icon tools (`Pencil`, `Eraser`, `Bucket`), stacked foreground/background swatches with swap control, removed explicit sub-grid/tile target toggles, and made tool behavior context-sensitive to the zoom canvas under the cursor.
- 2026-04-23: Refined paint toolbar visuals with tighter icon padding, custom eraser + half-tipped bucket icon styling, and compact FG/BG stacked swatches sized to match the square tool-button footprint.
- 2026-04-23: Switched paint tool glyphs to crisp inline SVG icons for well-known pencil/eraser/bucket controls, preserving the compact toolbar footprint and minimal padding.
- 2026-04-23: Replaced toolbar inline SVG approximations with exact source GIMP tool icons (`gimp-tool-pencil.svg`, `gimp-tool-eraser.svg`, `gimp-tool-bucket-fill.svg`) imported from GNOME `gimp-data` (`icons/Default/scalable`) and wired directly into Tile Zoom paint tool buttons.
- 2026-04-23: Updated the Tile Zoom color-picker control to the GIMP toolbox pattern by using overlapping FG/BG chips with exact GIMP `swap`/`default colors` icons (`gimp-swap-colors.svg`, `gimp-default-colors.svg`) from GNOME `gimp-data`, including click behavior to swap or reset FG/BG colors.
- 2026-04-23: Added a GIMP-style `Select` arrow tool button (`gimp-cursor.svg`) to Tile Zoom paint tools and changed tool-state fallback so deselecting `Pencil`/`Eraser`/`Bucket` always returns to active `Select` (arrow) mode.
- 2026-04-23: Replaced the Tile Zoom clear/trash button glyph with GNOME `gimp-data`'s `edit-delete.svg` (Delete Selected Layer style) to match icon style, and added a small top offset to visually separate the clear button from the main tool/color controls.
